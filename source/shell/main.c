...
static cli_t cli;
static const char *prompt = "sh >>"; // 命令行提示符
 ...
 * 命令行初始化
 */
static void cli_init(const char *prompt, const cli_cmd_t *cmd_list, int cnt)
{
    cli.prompt = prompt;
...
int main(int argc, char **argv)
{
    ...
    cli_init(prompt, cmd_list, sizeof(cmd_list) / sizeof(cli_cmd_t));
    for (;;)
    {
        // 显示提示符，开始工作
        show_prompt();
